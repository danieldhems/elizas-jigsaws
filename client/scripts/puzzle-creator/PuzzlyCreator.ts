...
import { Puzzle, PuzzleOrientation, PuzzleShapes } from '../types';
...
export default class PuzzlyCreator {
   ...
   constructor() {
      this.puzzleOptionsContainer = document.querySelector('#puzzle-setup--options') as this['puzzleOptionsContainer'];

      this.puzzleSizeInputField = document.querySelector('#puzzle-size-input-field') as this['puzzleSizeInputField'];
      this.puzzleSizeInputLabel = document.querySelector('#puzzle-size-input-label') as this['puzzleSizeInputLabel'];
      ...
      this.chkHighlights = document.querySelector('#chk-highlights') as this['chkHighlights'];
      this.chkNoDispersal = document.querySelector('#chk-no-disperse') as this['chkNoDispersal'];
      this.chkAddToLibrary = document.querySelector('#chk-add-to-library') as this['chkAddToLibrary'];
      this.imagePreviewElContainer = document.querySelector('#puzzle-setup--image') as this['imagePreviewEl'];
      this.imagePreviewEl = document.querySelector('#puzzle-setup--image_preview') as this['imagePreviewEl'];
      this.imagePreviewElInner = document.querySelector('#puzzle-setup--image_preview_inner') as this['imagePreviewEl'];
      this.imageUploadField = document.querySelector('#upload-fld') as this['imageUploadField'];
      this.newPuzzleForm = document.querySelector('#form-container') as this['newPuzzleForm'];
      this.startBtn = document.querySelector('#start-btn') as this['startBtn'];
      this.puzzleSizeField = document.getElementById('puzzle-size-input-field') as this['puzzleSizeField'];
      this.puzzleShapeInputFields = document.querySelectorAll(
         "[name='input-puzzle_shape']"
      ) as this['puzzleShapeInputFields'];
      this.imageUploadPreviewEl = document.getElementById(
         'puzzle-setup--image_preview-imgEl'
      ) as this['imageUploadPreviewEl'];
      this.puzzleTargetAreaElement = document.getElementById('image-crop') as this['puzzleTargetAreaElement'];
      ...
      if (imgId.length === 24) {
         console.log('Fetching image', imgId);
   ...
   addGeneralEventListeners() {
      this.imageUploadField.addEventListener('change', this.onImageUploadChange.bind(this));
   ...
   addPuzzleOptionEventListeners() {
      this.puzzleSizeInputField.addEventListener('input', (event: InputEvent) => {
         ...
         const highlightedPuzzleSize: Puzzle = this.activePuzzleConfigs[value];
         ...
         if (this.puzzleConfigs) {
            ...
            console.log('selected puzzle', this.selectedPuzzleConfig);

            this.PuzzleImpressionOverlay.setActiveImpression(this.selectedPuzzleConfig);
         }
      });
   ...
   onUploadSuccess(response: { data: SourceImage }) {
      ...
      if (response.data) {
         ...
         (this.imageUploadPreviewEl as HTMLImageElement).src = response.data.creatorImagePath;
   ...
   updatePuzzleSizeField(puzzleConfigs: Puzzle[]) {
      ...
      if (puzzleConfigs.length > 1) {
         ...
         this.puzzleSizeInputField.max = puzzleConfigs.length - 1 + '';
   ...
   setPuzzleImageOffsetAndWidth() {
      ...
      const cropLeftOffsetPercentage = Math.floor((leftPos / this.imageUploadPreviewEl.naturalWidth) * 100);
      const cropTopOffsetPercentage = Math.floor((topPos / this.imageUploadPreviewEl.naturalHeight) * 100);
      ...
      this.puzzleTargetAreaOffsetLeft = (this.imageUploadPreviewEl.naturalWidth / 100) * cropLeftOffsetPercentage;
      this.puzzleTargetAreaOffsetTop = (this.imageUploadPreviewEl.naturalHeight / 100) * cropTopOffsetPercentage;
      this.puzzleTargetAreaCalculatedWidth = (this.imageUploadPreviewEl.naturalWidth / 100) * cropWidthPercentage;
      this.puzzleTargetAreaCalculatedHeight = (this.imageUploadPreviewEl.naturalWidth / 100) * cropHeightPercentage;
   }
