import {
   EVENT_TYPES,
   HTML_ATTRIBUTE_NAME_SVG_PATH_STRING,
...
export default class SingleMovable extends BaseMovable {
   ...
   constructor({ puzzleData, pieceData }: { puzzleData: Puzzly; pieceData: PuzzlePiece }) {
      super(puzzleData);
   ...
   createElement() {
      ...
      el.setAttribute('data-pieces-per-side-horizontal', window.window.Puzzly.numberOfPiecesHorizontal + '');
      el.setAttribute('data-pieces-per-side-vertical', window.Puzzly.numberOfPiecesVertical + '');
      el.setAttribute('data-connects-to', JSON.stringify(this.getConnectingPieceIds(this.pieceData)));
      el.setAttribute('data-connections', JSON.stringify(window.Puzzly.GroupOperations.getConnections(el)));
      ...
      const svgOptions = {
         ...
         viewbox: `0 0 ${width + SHADOW_DISTANCE_FROM_PUZZLE_PIECE_IN_PX} ${
            height + SHADOW_DISTANCE_FROM_PUZZLE_PIECE_IN_PX
         }`
      };

      el.innerHTML = getSvg(`piece-${this.pieceData.index}`, [], puzzleImagePath, svgOptions);
   ...
   render() {
      ...
      if (Number.isInteger(pocketId) && pocketId !== -1) {
         const pocketElement = window.Puzzly.Pockets.container.querySelector(`#pocket-${pocketId}`);
   ...
   getConnectingPieceIds(data: Pick<PuzzlePiece, 'index' | 'pieceType'>) {
      ...
      switch (pieceType) {
         case PieceType.TopLeftCorner:
            return [index + 1, pieceBelowId];
         case PieceType.TopSide:
            return [index - 1, index + 1, pieceBelowId];
         case PieceType.TopRightCorner:
            return [index - 1, pieceBelowId];

         case PieceType.LeftSide:
            return [pieceAboveId, index + 1, pieceBelowId];
         case PieceType.Inner:
            return [pieceAboveId, index + 1, pieceBelowId, index - 1];
         case PieceType.RightSide:
            return [pieceAboveId, index - 1, pieceBelowId];
         case PieceType.BottomLeftCorner:
            return [pieceAboveId, index + 1];
         case PieceType.BottomSide:
            return [pieceAboveId, index - 1, index + 1];
   ...
   onMouseDown(event: MouseEvent) {
      if (event.button === 0) {
         if (
            ...
            (this.pocketId !== undefined && this.pocketId !== null && !Number.isNaN(this.pocketId))
         ) {
            return;
         }
